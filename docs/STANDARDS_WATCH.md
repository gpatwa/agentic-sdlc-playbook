...
one lands. Reviewed on a cadence (last: 2026-08-27). This file exists so
...
| MCP + A2A — **re-checked 2026-08-27**: both now sit under the Linux Foundation's Agentic AI Foundation (launched Dec 2025; co-founders OpenAI, Anthropic, Google, Microsoft, AWS, Block). MCP: 10,000+ enterprise servers, 97M+ SDK downloads. A2A: formal spec, in production at Microsoft/AWS/Salesforce/SAP/ServiceNow. Governance has converged; have not confirmed a specific *joint* MCP+A2A interop spec shipping beyond that — don't overclaim past what's checked | Maturity bar (`execution/ADAPTERS.md`'s reasoning) is closer to cleared than the 2026-08-09 check suggested. Our artefact-file handoffs remain the source of truth regardless; approval semantics still can't be expressed in-protocol | Confirm whether a specific joint interop spec (not just shared governance) has shipped → then consider an A2A adapter under `execution/<harness>/`. Still not before every agent runs in more than one harness |
| **Alternate agent harnesses** proliferating (DeepSeek `dsh`, Cursor, Codex, OpenCode, etc.) | Adapter *pattern* stays validated by their existence. **Re-checked 2026-08-27**: `dsh` still `0.1.0-rc.6`, zero commits since 2026-08-13 — unchanged, still developer-preview. Full reasoning + the Agent Skills finding (a different primitive, not a shortcut around a second adapter) now lives in `docs/BACKLOG.md`'s parked "second harness adapter" bullet and T8 — see there rather than duplicating | Same trigger, unrevised: a second `execution/<harness>/` adapter only when the first is proven **and** a concrete need appears |
